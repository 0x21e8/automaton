...
use crate::domain::types::{
    ...
    ExposureReconciliationStatus, InboxMessage, InboxStats, InferenceConfigView,
    InferenceProvider, InferenceProxyStatusView, MemoryFact, MemoryRollup,
    ObservabilitySnapshot, OpenRouterProxyWorkerConfig, OpenRouterReasoningLevel, OutboxMessage,
    OutboxStats, PendingStrategyDiscoveryJob, PromptLayer, PromptLayerView,
    PromoteDiscoveryProtocolArtifactsArgs, ReflectionMemoryRecord, RetentionConfig,
    ...
    SubmitInferenceResultArgs, SubmitStrategyDiscoveryResultArgs, TaskKind, TaskScheduleConfig,
...
fn steward_command_label(command: &StewardCommand) -> &'static str {
    match command {
        ...
        StewardCommand::SetStrategyDiscoveryWorkerConfig { .. } => "set_strategy_discovery_worker_config",
        ...
        StewardCommand::PromoteDiscoveryProtocolArtifacts { .. } => "promote_discovery_protocol_artifacts",
...
async fn enqueue_strategy_discovery_job_shared(
    ...
    let objective = args
        ...
        .trim()
        .to_string();
    let watchlist = args.watchlist.unwrap_or_else(|| config.protocol_watchlist.clone());
...
async fn dispatch_steward_command(
    ...
    match command {
        ...
        StewardCommand::EnqueueStrategyDiscoveryJob { args } => {
            ...
            Ok(format!("strategy_discovery_job_enqueued job_id={}", pending.job_id))
        }
