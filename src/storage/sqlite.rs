...
use crate::domain::types::{
    ...
    OutboxMessage, PendingStrategyDiscoveryJob, ReflectionMemoryRecord, RuntimeSnapshot,
    ScheduledJob, SchedulerRuntime, SkillRecord, StrategyDiscoveryCallbackRecord,
...
const MIGRATION_006_STRATEGY_DISCOVERY_SCHEMA: &str = r#"
...
mod backend {
    use super::{
        ...
        MIGRATION_006_STRATEGY_DISCOVERY_SCHEMA,
    };
    ...
    fn apply_migrations(conn: &Connection) -> SqlResult<()> {
        ...
        conn.execute_batch(MIGRATION_005_AUTONOMY_RUNTIME_SCHEMA)
            .map_err(|err| err.to_string())?;
        conn.execute_batch(MIGRATION_006_STRATEGY_DISCOVERY_SCHEMA)
            .map_err(|err| err.to_string())?;
...
pub fn strategy_discovery_pending_job(
    ...
    backend::with_connection(|conn| {
        let mut stmt = conn
            .prepare(
                "SELECT payload_json FROM strategy_discovery_pending_jobs WHERE job_id = ?1",
            )
            .map_err(|err| err.to_string())?;
        let mut rows = stmt
            .query_map([trimmed], |row| row.get::<_, String>(0))
...
pub fn strategy_discovery_results_count() -> Result<u64, String> {
    backend::with_connection(|conn| {
        let count: i64 = conn
            .query_row("SELECT COUNT(1) FROM strategy_discovery_results", [], |row| {
                row.get(0)
            })
            .map_err(|err| err.to_string())?;
        Ok(count.max(0) as u64)
