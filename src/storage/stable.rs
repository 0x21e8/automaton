...
use crate::domain::types::{
    ...
    OutboxStats, PendingInferenceProxyJob, PendingStrategyDiscoveryJob, PromptLayer,
    PromptLayerView, ProtocolWatchlistEntry, ReflectionMemoryRecord, ReflectionOrigin,
    RetentionConfig, RetentionMaintenanceRuntime, RoomMessage, RoomPollingState,
    ...
    StorageGrowthMetrics, StoragePressureLevel, StrategyDiscoveryCallbackRecord,
    StrategyDiscoveryResultPayload, StrategyDiscoveryResultStatus, StrategyDiscoveryStatusView,
    StrategyDiscoveryWorkerConfig, StrategyKillSwitchState, StrategyOutcomeEvent,
    StrategyOutcomeKind, StrategyOutcomeStats, StrategyQuarantine, StrategyTemplate,
    StrategyTemplateKey, SubmitInferenceResultArgs, SubmitStrategyDiscoveryResultArgs,
...
pub fn set_strategy_discovery_worker_config(
    ...
    if config.enabled {
        if worker_base_url.is_empty() {
            return Err("strategy discovery worker_base_url cannot be empty when enabled".to_string());
        }
        ...
        if protocol_watchlist.is_empty() {
            return Err("strategy discovery protocol_watchlist cannot be empty when enabled".to_string());
        }
    }
...
pub fn assert_strategy_discovery_callback_authorized(
    caller_principal: &str,
) -> Result<(), String> {
    let expected = runtime_snapshot()
        .strategy_discovery_worker
        .trusted_callback_principal
        .ok_or_else(|| "strategy discovery trusted callback principal is not configured".to_string())?
        .to_text();
    if caller_principal != expected {
        return Err(format!(
...
fn strategy_discovery_watchlist_hash(watchlist: &[ProtocolWatchlistEntry]) -> Result<String, String> {
    let payload =
        serde_json::to_vec(watchlist).map_err(|error| format!("watchlist hash encode failed: {error}"))?;
...
fn validate_strategy_discovery_payload(
    ...
    {
        return Ok(StrategyDiscoveryResultStatus::Rejected {
            reason: "strategy discovery callback payload must include artifacts, candidates, or sources".to_string(),
        });
    }
    ...
    for bundle in &args.payload.protocol_artifacts {
        if bundle.bundle_id.trim().is_empty() {
            return Ok(StrategyDiscoveryResultStatus::Rejected {
                reason: "strategy discovery protocol artifact bundle_id cannot be empty".to_string(),
            });
        }
...
pub fn upsert_pending_strategy_discovery_job(job: PendingStrategyDiscoveryJob) -> Result<(), String> {
...
pub fn get_pending_strategy_discovery_job(job_id: &str) -> Option<PendingStrategyDiscoveryJob> {
    sqlite::strategy_discovery_pending_job(job_id).ok().flatten()
}
...
pub fn apply_strategy_discovery_callback(
    ...
    let pending_job = sqlite::strategy_discovery_pending_job(&args.job_id).ok().flatten();
...
pub fn freshest_validated_strategy_discovery_result_for_config(
    ...
    let expected_watchlist_hash = strategy_discovery_watchlist_hash(&config.protocol_watchlist).ok()?;
...
pub fn strategy_discovery_status_view(now_ns_param: u64) -> StrategyDiscoveryStatusView {
    ...
    let freshest_validated_age_secs = freshest_validated_at_ns
        .map(|value| now_ns_param.saturating_sub(value) / 1_000_000_000);
...
mod tests {
    ...
    fn strategy_discovery_storage_roundtrip_and_dedupe() {
        ...
        assert!(matches!(duplicate, StrategyDiscoveryCallbackApply::Duplicate));
