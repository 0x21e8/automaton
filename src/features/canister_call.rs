...
pub async fn canister_call_tool(args_json: &str) -> Result<String, String> {
    let args: CanisterCallArgs =
        serde_json::from_str(args_json).map_err(|e| format!("invalid canister_call args: {e}"))?;
...
async fn call_canister_raw(
    ...
    let response_bytes = do_call(canister_id, method, encoded).await?;
    ...
    let truncated = if result_text.chars().count() > MAX_CANISTER_CALL_OUTPUT_CHARS {
        let cut: String = result_text
            .chars()
            .take(MAX_CANISTER_CALL_OUTPUT_CHARS)
            .collect();
        format!("{}... [truncated, {} total chars]", cut, result_text.len())
    } else {
        result_text
    };
    ...
    #[cfg(test)]
    {
        MOCK_CANISTER_CALL.with(|m| {
            m.borrow()
                ...
                .unwrap_or_else(|| Err(format!("no mock registered for {_canister_id}.{_method}")))
        })
    }
    #[cfg(not(test))]
    {
...
#[cfg(test)]
type MockCanisterCallFn = dyn Fn(Principal, &str, &[u8]) -> Result<Vec<u8>, String>;
