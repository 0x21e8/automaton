...
function defaultDetail(
  ...
  return {
    ...
    generation: registryRecord?.generation ?? 0,
    parentConstitutionHash: registryRecord?.parentConstitutionHash ?? null,
...
export function normalizeAutomatonDetail(options: {
  ...
  return {
    ...
    metabolism: {
      ...
      lifetimePatronageUsdcRaw: runtime?.snapshot.runtime?.lifetime_patronage_usdc_raw ??
        base.metabolism?.lifetimePatronageUsdcRaw ?? "0",
    ...
    generation: options.registryRecord?.generation ?? base.generation ?? 0,
    parentConstitutionHash: options.registryRecord?.parentConstitutionHash ?? base.parentConstitutionHash ?? null,
