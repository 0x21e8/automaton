...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    *)
      ...
      echo "usage: npm run eval:dev -- --experiment evaluations/experiments/<name>.yaml" >&2
      exit 1
      ;;
  esac
done
...
web_pid=""
launchpad_pid=""
backend_log="$EVALUATOR_LOG_DIR/backend.dev.log"
web_log="$EVALUATOR_LOG_DIR/web.dev.log"
launchpad_log="$EVALUATOR_LOG_DIR/launchpad.dev.log"
last_reported_state=""
last_terminal_state=""
...
printf '%s\n' "starting evaluation harness in dev mode"
printf '  experiment: %s\n' "$experiment_path"
printf '  evaluator:  %s\n' "$EVALUATOR_BASE_URL"
...
EVALUATOR_ARTIFACTS_ROOT="$EVALUATOR_ARTIFACTS_ROOT" \
  npm run dev --workspace @ic-automaton/evaluator -- --experiment "$experiment_path" >"$backend_log" 2>&1 &
backend_pid=$!

VITE_EVALUATOR_BASE_URL="$EVALUATOR_BASE_URL" \
  npm exec --workspace @ic-automaton/evaluator-web vite -- --host "$EVALUATOR_WEB_HOST" --port "$EVALUATOR_WEB_PORT" --strictPort >"$web_log" 2>&1 &
web_pid=$!

VITE_INDEXER_BASE_URL="$LAUNCHPAD_INDEXER_BASE_URL" \
  npm exec --workspace @ic-automaton/web vite -- --host "$LAUNCHPAD_WEB_HOST" --port "$LAUNCHPAD_WEB_PORT" --strictPort >"$launchpad_log" 2>&1 &
launchpad_pid=$!
    ...
    if [ -n "$dashboard_json" ]; then
      ...
      case "$current_state" in
        completed|aborted|failed)
          if [ "$current_state" != "$last_terminal_state" ]; then
            print_terminal_summary "$dashboard_json"
            last_terminal_state=$current_state
          fi
          ;;
      esac
    fi
